...
//
//        // Number of User Like
//        long numUsersLike = geoplatformServiceClient.getUsersCount(new SearchRequest(usernameTest));
//        Assert.assertEquals("Number of User Like", numUsersLike, new Long(1).longValue());
//
//        // Get User from Id
//        try {
//            // Get UserDTO from Id
//            UserDTO userDTOFromWS = geoplatformServiceClient.getShortUser(new RequestById(idUserTest));
//            Assert.assertNotNull(userDTOFromWS);
//            Assert.assertEquals("Error found User from Id", idUserTest, userDTOFromWS.getId());
//            // Get GPUser from Id
//            GPUser userFromWS = geoplatformServiceClient.getUserDetail(new RequestById(idUserTest));
...
//            UserDTO userDTOFromWS = geoplatformServiceClient.getShortUserByName(new SearchRequest(usernameTest));
//            Assert.assertNotNull(userDTOFromWS);
//            Assert.assertEquals("Error found User from Username", idUserTest, userDTOFromWS.getId());
//            // Get GPUser from Username
//            GPUser userFromWS = geoplatformServiceClient.getUserDetailByName(new SearchRequest(usernameTest));
//            Assert.assertNotNull(userFromWS);
...
//        GPUser user = null;
//        try {
//            user = geoplatformServiceClient.getUserDetailByUsernameAndPassword(usernameTest, "pwd_username_test_ws");
...
//
    ...
    public void testGetUserDetailByUsernameAndPassword2() {
        ...
        try {
            ...
            user = geoplatformServiceClient.getUserDetailByUsernameAndPassword(newUsername, "pwd_username_test_ws");
    ...
    public void testGetUserDetailByUsernameAndPassword3() {
        GPUser user = null;
        try {
            user = geoplatformServiceClient.getUserDetailByUsernameAndPassword(usernameTest, "pwd_username_test_ws_");
